...
export function focusPrio(ns, focusOn) {
    ...
    if (ns.singularity.isBusy()) {
        ...
        if (work.type === "CREATE_PROGRAM") {
            ...
            if (["CREATE_PROGRAM"].includes(focusOn)) { return true } else { return false }
            ...
            if (["str", "def", "dex", "agi"].includes(work.classType)) {
                ...
                if (["SCHOOL", "GYM"].includes(focusOn)) { return true } else { return false }

            } else {

                ns.print("At school working on " + work.classType)
                if (["SCHOOL"].includes(focusOn)) { return true } else { return false }
            }

        } else if (work.type === "COMPANY") {

            ns.print("Working a job at " + work.companyName)
            if (["CREATE_PROGRAM", "SCHOOL", "GYM", "COMPANY"].includes(focusOn)) { return true } else { return false }

        } else if (work.type === "CRIME") {

            ns.print("Attempting to " + work.crimeType)
            if (["CREATE_PROGRAM", "SCHOOL", "GYM", "COMPANY", "CRIME"].includes(focusOn)) { return true } else { return false }

        } else if (work.type === "FACTION") {

            ns.print("Working with " + work.factionName)
            if (["CREATE_PROGRAM", "SCHOOL", "GYM", "COMPANY", "CRIME", "FACTION"].includes(focusOn)) { return true } else { return false }
